...
object Exercises {
    ...
    def sumOfDivBy3Or5(iFrom: Int, iTo: Int): Long = {
        var sum = 0;
        (iFrom to iTo)
          .foreach(i => if (i % 3 == 0 || i % 5 == 0) sum += i)
        sum
    }
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        val ballsProportions = for ((materials, parameters) <- ballsArray) yield (materials, math.pow(parameters._1, 3) * parameters._2)
        ballsProportions.toSeq.sortBy(_._2).toMap.keys.toSeq
    }
